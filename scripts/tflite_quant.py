...
def main(cfg: DictConfig) -> None:
            ...
            for image_file in tqdm.tqdm(list_of_files):
                if image_file.endswith(".jpg"):
                    image = cv2.imread(os.path.join(
                        representative_ds_path, image_file))
                    if len(image.shape) != 3:
                        image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
                    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                    resized_image = cv2.resize(image, (int(cfg.model.input_shape[0]), int(
                        cfg.model.input_shape[1])), interpolation=cv2.INTER_LINEAR)
                    image_data = resized_image/cfg.pre_processing.rescaling.scale + \
    ...
    tflite_model_quantio_file = tflite_models_dir / \
        f'{name}_{quant_tag}_{input_tag}{output_tag}_{uc}.tflite'
    tflite_model_quantio_file.write_bytes(tflite_model_quantio)

    print(
        f'Quantized model generated: {name}_{quant_tag}_{input_tag}{output_tag}_{uc}.tflite')
